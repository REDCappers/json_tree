...
def draw_tree(data, retsu=1):
    ...
    if type(data) == dict:
        for k in data.keys():
            ...
            # 配列の先頭、末尾判定
            if next(iter(reversed(data))) == k and check_last_color:
...
def main(import_json_path: str, output_excel_path: str, excel_last_color_toggle_bool: bool, excel_open_bool: bool):
    """
    メイン処理
    """
    file_path = import_json_path
    global gyou
    gyou = 1

    global check_last_color
    check_last_color = excel_last_color_toggle_bool
